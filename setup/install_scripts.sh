...
# CRON jobs for DHIS2
PATH=/snap/bin:/usr/local/sbin:${DHIS2_BIN_DIR}:/sbin:/bin:/usr/sbin:/usr/bin

# Run backup script at 8:25pm (adjust and uncomment) 
# 25 20 * * *     root ${DHIS2_BIN_DIR}/dhis2-backup
EOF
fi
...
# copy containers.json to /usr/local/etc/dhis/ if doesn't exist
if [ -f ${DHIS2_CONFIG_DIR}/containers.json ]; then
  echo "containers.json already exists, not over-writing"
elif [ -f configs/containers.json ]; then
  cp configs/containers.json $DHIS2_CONFIG_DIR
else
  echo "configs/containers.json or ${DHIS2_CONFIG_DIR}/containers.json configuration file does not exist. Create a configuration file to continue."
  exit 1
fi
